...
NSString *const kHomeUrl = @"http://Talk2Me.com";
NSString *const kLogoUrl = @"http://files.quickblox.com/ic_launcher.png";
NSString *const kAppName = @"Talk2Me";
...
+ (void)fetchMyFriends:(void(^)(NSArray *facebookFriends))completion {
    ...
    [friendsRequest startWithCompletionHandler:^(FBRequestConnection *connection, id result, NSError *error) {
        NSArray *myFriends = error ? @[] : [(FBGraphObject *)result objectForKey:kDataKey];
...
+ (void)shareToUsers:(NSString *)usersIDs completion:(void(^)(NSError *error))completion {
    ...
    NSDictionary *postParams = @{
                                 ...
                                 @"description" : @"",
                                 @"place":@"155021662189",
                                 @"message": NSLocalizedString(@"STR_DEAR_FRIEND", nil),
                                 @"tags" : usersIDs
                                 };
    ...
    if (error) {
        ...
        if (shouldNotify) {
            ...
            if (errorCategory == FBErrorCategoryUserCancelled) {
                ...
                alertText = NSLocalizedString(@"STR_YOUR _CURRENT_SESSION_IS_NO_LONGER_VALID", nil);
