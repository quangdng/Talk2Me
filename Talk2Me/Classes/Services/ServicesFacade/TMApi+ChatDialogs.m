...
NSString const *kEditDialogExtendedNameParameter = @"name";
NSString const *kEditDialogExtendedPushOccupantsParameter = @"push[occupants_ids][]";
NSString const *kEditDialogExtendedPullOccupantsParameter = @"pull_all[occupants_ids][]";

@interface TMApi()

@end

@implementation TMApi (ChatDialogs)
...
- (void)createChatDialog:(QBChatDialog *)chatDialog occupants:(NSArray *)occupants completion:(QBChatDialogResultBlock)completion {
    ...
    [self.chatDialogsService createChatDialog:chatDialog completion:^(QBChatDialogResult *result) {
        ...
        if ([weakSelf checkResult:result]) {
            ...
            [weakSelf sendNotificationWithType:MessageNotificationTypeCreateDialog
...
- (QBChatMessage *)notification:(MessageNotificationType)type recipient:(QBUUser *)recipient text:(NSString *)text chatDialog:(QBChatDialog *)chatDialog {
...
- (void)sendNotificationWithType:(MessageNotificationType)type text:(NSString *)text toRecipients:(NSArray *)recipients chatDialog:(QBChatDialog *)chatDialog {
...
- (void)changeChatName:(NSString *)dialogName forChatDialog:(QBChatDialog *)chatDialog completion:(QBChatDialogResultBlock)completion {
    ...
    extendedRequest[kEditDialogExtendedNameParameter] = dialogName;
    ...
    [self.chatDialogsService updateChatDialogWithID:chatDialog.ID extendedRequest:extendedRequest completion:^(QBChatDialogResult *result) {
        ...
        if ([weakSelf checkResult:result]) {
            ...
            [weakSelf sendNotificationWithType:MessageNotificationTypeUpdateDialog
...
- (void)joinOccupants:(NSArray *)occupants toChatDialog:(QBChatDialog *)chatDialog completion:(QBChatDialogResultBlock)completion {
    ...
    NSMutableDictionary *extendedRequest = [[NSMutableDictionary alloc] init];
    extendedRequest[kEditDialogExtendedPushOccupantsParameter] = occupantsToJoinIDs;
...
- (void)leaveWithUserId:(NSUInteger)userID fromChatDialog:(QBChatDialog *)chatDialog completion:(QBChatDialogResultBlock)completion {
    ...
    NSMutableDictionary *extendedRequest = [[NSMutableDictionary alloc] init];
    extendedRequest[kEditDialogExtendedPullOccupantsParameter] = [NSString stringWithFormat:@"%d", userID];
