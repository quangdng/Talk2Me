...
+ (void)setupWithName:(NSString *)name {
    ...
    storage = [[DBStorage alloc] initWithStack:stack storeName:name];
}
...
- (instancetype)initWithStack:(MagicalRecordStack *)stack storeName:(NSString *)storeName {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create("com.Talk2Me.DBQueue", NULL);
        self.stack = stack;
...
@implementation NSObject (DBStorage)

@dynamic dbStorage;

- (DBStorage *)dbStorage {
    
    return [DBStorage shared];
}
